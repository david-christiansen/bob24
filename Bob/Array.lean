...
--
-- * `Array.push` will mutate the array in place if the array value's reference count is exactly 1,
--   so this code's seeming use of mutation becomes real mutation, with accidental aliasing leading
--   to extra copying instead of scary bugs
...
-- This version of `filter` is easier to prove things about. `do` notation is nice, but the
-- desugared programs are more difficult to use in verification than hand-written
...
-- Prove that the inner loop in `filter` ensures that the predicate
-- holds for its result, on the assumption that it holds for all
-- elements in the initial accumulator.
--
-- The following tactics may be useful:
--  * `unfold f` replaces `f` with its definition in the goal
--  * `split` replaces a goal that contains an `if` or `match`
--    expression with a new goal for each branch
-- * `apply` applies an existing lemma
-- * `assumption` proves the goal using a hypothesis if possible
--
-- Because the proof should follow the recursive structure of the
-- program, you may additionally need to copy the termination argument
-- from the program to the proof.
